...
import tov_vectorized # Custom import
...
def TOV2(logrho_c_array, theta):
    ...
    for r in rho_samples:
        ...
        p_eos.append(pv * geom_conv)
        e_eos.append(ev * geom_conv)
    
    # 2. Central Pressures
    pc_geom = [p_eps_of_rho(10**lr, logp, g1, g2, g3)[0]*geom_conv for lr in logrho_c_array]
    
    # 3. Batch Solve
    m, r, td = tov_vectorized.tov_mrl_vector(np.array(e_eos), np.array(p_eos), np.array(pc_geom))
...
def benchmark(model, sampled_combinations, logrho_c_array,
    ...
    for (Gamma1, Gamma2, Gamma3, logp) in sampled_combinations:
        ...
        # Pre-build input to measure pure inference (optional)
        # Or keep inside to measure full overhead
